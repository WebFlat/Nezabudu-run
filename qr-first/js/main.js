...
jQuery(function ($) {
	...
	$('#datepicker').Zebra_DatePicker({
		...
		direction: ['2021-03-01', '2021-05-30'],
...
function validateTelHelps() {
    var reg = /^\+380\d{3}\d{2}\d{2}\d{2}$/;
    var telHelps = $('#connect').val();
    if (reg.test(telHelps) == false) {
        return false;
    }
}
function validateNameHelps() {
    var reg = /^[А-Яа-яЁё\s]+$/;
    var nameHelps = $('#con-name').val();
    if (reg.test(nameHelps) == false) {
        return false;
    }
}


$('#sendHelps').click(function (e) {
    e.preventDefault();
    var tel = $('#connect');
    var name = $('#con-name');
    var message = $('.alert__mess');
    if (name.val() == '' || validateNameHelps() == false || tel.val() == '' || validateTelHelps() == false) {
        message.html('<p class="alert__text">Данные введены не верно!</p>').show().delay(4000).fadeOut(300);
        tel.val('');
        return false;
    } else {
        $.ajax({
            ...
            success: function (data) {
                message.html('<p class="alert__text">Запрос отправлен, мы скоро свяжемся с Вами</p>').show().delay(4000).fadeOut(300);
                tel.val('');
                name.val('');
                setTimeout(function () {
                ...
                message.html('<p class="alert__text"">Ошибка отправки!</p>').show().delay(4000).fadeOut(300);
                tel.val('');
                name.val('');
            }
        })
...
$('#sendOrder').click(function (e) {
    ...
    if (sector.val() == '' || area.val() == '' || row.val() == '' || number.val() == '' || validateName(name) == false || validateTel(userTel) == false || validateName(surname) == false || validateName(patronymic) == false || validateName(userSurname) == false || validateName(userName) == false || validateMail(userEmail) == false || validateName(cemetery) == false) {
        message.html('<p class="alert-order__text">Данные не заполнены или введены не верно!</p>').show().delay(2000).fadeOut(300);
        setTimeout(function () {
            cemetery.focus();
        }, 2500);
        return false;
    } else if (order.val() == 0 || order.val() == undefined) {
        message.html('<p class="alert-order__text">Сделайте заказ!</p>').show().delay(2000).fadeOut(300);
    } else if (date.val() == '') {
        message.html('<p class="alert-order__text">Выберите предварительную дату!</p>').show().delay(2000).fadeOut(300);
    } else {
        $.ajax({
            type: "POST",
            url: "sendOrder.php",
            ...
            success: function (data) {
                message.html('<p class="alert-order__text">Заказ отправлен, мы скоро свяжемся с Вами для подтверждения</p>').show().delay(4000).fadeOut(300);
                // setTimeout(function () {
                //     window.location.replace("../../index.html");
                ...
                message.html('<p class="alert-order__text">Ошибка отправки!</p>').show().delay(4000).fadeOut(300);
            }
        })
    }
})
